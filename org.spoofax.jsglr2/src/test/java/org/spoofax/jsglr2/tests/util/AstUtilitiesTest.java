...
public class AstUtilitiesTest {
	...
	public void testExpansionListExpansion() throws ParseError, ParseTableReadException, IOException {
		...
		List<List<String>> expectedExpansion = Arrays.asList(
			Arrays.asList("a", "b1", "c"),
			Arrays.asList("a", "b2", "c")
		);
	...
	public void testCountAmbShared() throws ParseError, ParseTableReadException, IOException {
		...
		IStrategoTerm termAmb1or2 = termFactory.makeAppl(termFactory.makeConstructor("amb", 1), new IStrategoTerm[]{termFactory.makeList(new IStrategoTerm[]{term1, term2})});
		...
		IStrategoTerm termAmbTop = termFactory.makeAppl(termFactory.makeConstructor("amb", 1), new IStrategoTerm[]{termFactory.makeList(new IStrategoTerm[]{termTupleA1or2, termTuple1or2B})});
	...
	public void testFlattenAmb() throws ParseError, ParseTableReadException, IOException {
		...
		IStrategoTerm termAmb1or2 = termFactory.makeAppl(termFactory.makeConstructor("amb", 1), new IStrategoTerm[]{termFactory.makeList(new IStrategoTerm[]{term1, term2})});
		IStrategoTerm termAmbTop = termFactory.makeAppl(termFactory.makeConstructor("amb", 1), new IStrategoTerm[]{termFactory.makeList(new IStrategoTerm[]{termA, termAmb1or2})});
