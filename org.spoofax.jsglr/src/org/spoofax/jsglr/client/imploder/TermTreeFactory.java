...
public class TermTreeFactory implements ITreeFactory<IStrategoTerm> {
	...
	public IStrategoTerm createNonTerminal(String sort, String constructor,
			IToken leftToken, IToken rightToken, List<IStrategoTerm> children, boolean isCompletion, boolean isNestedCompletion) {
	...
	public IStrategoTerm createIntTerminal(String sort, IToken token, int value) {
		...
		configure(result, sort, token, token, false, false, false, false);
		return result;
	}
	...
	public IStrategoTerm createStringTerminal(String sort, IToken leftToken, IToken rightToken, String value) {
		...
		configure(result, sort, leftToken, rightToken, false, false, false, false);
		return result;
	}
	...
	public IStrategoTerm createTuple(String elementSort, IToken leftToken,
		...
		configure(result, elementSort, leftToken, rightToken, true, false, false, false);
		return result;
	}

	public IStrategoTerm createAmb(List<IStrategoTerm> alternatives, IToken leftToken, IToken rightToken, boolean isCompletion, boolean isNestedCompletion) {
		...
		return createNonTerminal(null, "amb", leftToken, rightToken, alternativesInList, isCompletion, isNestedCompletion);
	}
	...
	public IStrategoTerm recreateNode(IStrategoTerm oldNode, IToken leftToken, IToken rightToken, List<IStrategoTerm> children) {
		...
		switch (oldNode.getTermType()) {
			...
			case APPL:
				return createNonTerminal(getSort(oldNode), ((IStrategoAppl) oldNode).getName(), leftToken, rightToken, children, it.isCompletion(), it.isNestedCompletion());
			case LIST:
	...
	public IStrategoTerm createInjection(String sort, IToken leftToken, IToken rightToken, List<IStrategoTerm> children, boolean isCompletion, boolean isNestedCompletion, boolean isBracket) {
		if (children.size() == 1) {
		    ...
		    if (isBracket) {
	            ...
	            if (ImploderAttachment.get(result).isCompletion()) {
                    configure(result, result.getAttachment(ImploderAttachment.TYPE).getSort(), left, right, false, isBracket, true, false);
                } else {
                    configure(result, result.getAttachment(ImploderAttachment.TYPE).getSort(), left, right, false, isBracket, isCompletion, isNestedCompletion);
                }    
	            return result;    
		    }
		    else {
	        ...
	        if(child instanceof IStrategoAppl){
                if (((IStrategoAppl) child).getConstructor().getName().equals("amb")){
                    ...
                    if (ImploderAttachment.get(child).isCompletion()) {
                        configure(child, child.getAttachment(ImploderAttachment.TYPE).getSort(), left, right, false, isBracket, true, false);
                    } else {
                        configure(child, child.getAttachment(ImploderAttachment.TYPE).getSort(), left, right, false, isBracket, isCompletion, isNestedCompletion);
                    }                                  
                }
	        } 
        }        
	...
	protected void configure(IStrategoTerm term, String sort, IToken leftToken, IToken rightToken, boolean isListOrTuple, boolean isBracket, boolean isCompletion, boolean isNestedCompletion) {
		assert isListOrTuple
			== (term.getTermType() == TUPLE || term.getTermType() == LIST);
		if (enableTokens)
			putImploderAttachment(term, isListOrTuple, sort, leftToken, rightToken, isBracket, isCompletion, isNestedCompletion);
	}
}
