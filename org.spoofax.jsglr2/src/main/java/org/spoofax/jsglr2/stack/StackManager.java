...
public abstract class StackManager<StackNode extends AbstractStackNode<ParseForest>, ParseForest extends AbstractParseForest> {
    ...
    public abstract StackLink<StackNode, ParseForest> createStackLink(Parse<StackNode, ParseForest> parse,
    ...
    private void findAllPathsOfLength(StackPath<StackNode, ParseForest> path, int length,
        List<StackPath<StackNode, ParseForest>> paths) {
        if(length == 0)
            paths.add(path);
        else {
            ...
            for(StackLink<StackNode, ParseForest> linkOut : stackLinksOut(lastStackNode)) {
                StackPath<StackNode, ParseForest> extendedPath =
                    new NonEmptyStackPath<StackNode, ParseForest>(linkOut, path);
    ...
    public ParseForest[] getParseForests(ParseForestManager<ParseForest, ?, ?> parseForestManager,
        StackPath<StackNode, ParseForest> pathBegin) {
        ...
        if(res != null) {
            ...
            for(int i = 0; i < pathBegin.length; i++) {
                NonEmptyStackPath<StackNode, ParseForest> nonEmptyPath =
                    (NonEmptyStackPath<StackNode, ParseForest>) path;
