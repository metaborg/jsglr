...
public class NewStructureSkipper {
    ...
    private ArrayList<Integer> findCurrentEnd(int indexStartLine) {
        ...
        while(myParser.getCurrentToken().getToken()!=SGLR.EOF && 0<=indexNextLine && indexNextLine<=getHistory().getIndexLastLine()){            
    ...
    private String readLine(int index) {
        while(getHistory().getIndexLastLine()<=index && myParser.getCurrentToken().getToken()!=SGLR.EOF)
    ...
    private int skipLine(int indexLine) {
        ...
        getHistory().readRecoverToken(myParser, false);
        while(myParser.getCurrentToken().getToken()!=SGLR.EOF){
            getHistory().readRecoverToken(myParser, false);            
            skipIndentHandler.updateIndentation(myParser.getCurrentToken().getToken());
