        ...
        switch(this.parseForestRepresentation) {
            ...
            case Basic: switch(this.reducing) {
                case Elkhound: switch(this.stackRepresentation) {
                    case BasicElkhound:
                        if (this.recovery)
                            return    withRecovery(new ElkhoundParser<>(inputStackFactory, RecoveryParseState.factory(activeStacksFactory, forActorStacksFactory), parseTable, BasicElkhoundStackManager.factory(),  BasicParseForestManager.factory(), null, ElkhoundReduceManager.factoryElkhound(recoveryReducerFactory()), RecoveryParseFailureHandler.factory(), RecoveryParseReporter.factory()));
                        else
                            return withoutRecovery(new ElkhoundParser<>(inputStackFactory, ParseState.factory(activeStacksFactory, forActorStacksFactory),         parseTable, BasicElkhoundStackManager.factory(),  BasicParseForestManager.factory(), null, ElkhoundReduceManager.factoryElkhound(reducerFactory()),         DefaultParseFailureHandler.factory(),  EmptyParseReporter.factory()));
                    case HybridElkhound:
                        if (this.recovery)
                            return    withRecovery(new ElkhoundParser<>(inputStackFactory, RecoveryParseState.factory(activeStacksFactory, forActorStacksFactory), parseTable, HybridElkhoundStackManager.factory(), BasicParseForestManager.factory(), null, ElkhoundReduceManager.factoryElkhound(recoveryReducerFactory()), RecoveryParseFailureHandler.factory(), RecoveryParseReporter.factory()));
                        else
                            return withoutRecovery(new ElkhoundParser<>(inputStackFactory, ParseState.factory(activeStacksFactory, forActorStacksFactory),         parseTable, HybridElkhoundStackManager.factory(), BasicParseForestManager.factory(), null, ElkhoundReduceManager.factoryElkhound(reducerFactory()),         DefaultParseFailureHandler.factory(),  EmptyParseReporter.factory()));
                ...
                case Basic: switch(this.stackRepresentation) {
                    case Basic:
                        if (this.recovery)
                            return    withRecovery(new Parser<>(inputStackFactory, RecoveryParseState.factory(activeStacksFactory, forActorStacksFactory), parseTable, BasicStackManager.factory(),  BasicParseForestManager.factory(), null, ReduceManager.factory(recoveryReducerFactory()), RecoveryParseFailureHandler.factory(), RecoveryParseReporter.factory()));
                        else
                            return withoutRecovery(new Parser<>(inputStackFactory, ParseState.factory(activeStacksFactory, forActorStacksFactory),         parseTable, BasicStackManager.factory(),  BasicParseForestManager.factory(), null, ReduceManager.factory(reducerFactory()),         DefaultParseFailureHandler.factory(),  EmptyParseReporter.factory()));
                    case Hybrid:
                        if (this.recovery)
                            return    withRecovery(new Parser<>(inputStackFactory, RecoveryParseState.factory(activeStacksFactory, forActorStacksFactory), parseTable, HybridStackManager.factory(), BasicParseForestManager.factory(), null, ReduceManager.factory(recoveryReducerFactory()), RecoveryParseFailureHandler.factory(), RecoveryParseReporter.factory()));
                        else
                            return withoutRecovery(new Parser<>(inputStackFactory, ParseState.factory(activeStacksFactory, forActorStacksFactory),         parseTable, HybridStackManager.factory(), BasicParseForestManager.factory(), null, ReduceManager.factory(reducerFactory()),         DefaultParseFailureHandler.factory(),  EmptyParseReporter.factory()));
            ...
            case Null: switch(this.reducing) {
                case Elkhound: switch(this.stackRepresentation) {
                    case BasicElkhound:
                        if (this.recovery)
                            return    withRecovery(new ElkhoundParser<>(inputStackFactory, RecoveryParseState.factory(activeStacksFactory, forActorStacksFactory), parseTable, BasicElkhoundStackManager.factory(),  NullParseForestManager.factory(), null, ElkhoundReduceManager.factoryElkhound(recoveryReducerFactory()), RecoveryParseFailureHandler.factory(), RecoveryParseReporter.factory()));
                        else
                            return withoutRecovery(new ElkhoundParser<>(inputStackFactory, ParseState.factory(activeStacksFactory, forActorStacksFactory),         parseTable, BasicElkhoundStackManager.factory(),  NullParseForestManager.factory(), null, ElkhoundReduceManager.factoryElkhound(reducerFactory()),         DefaultParseFailureHandler.factory(),  EmptyParseReporter.factory()));
                    case HybridElkhound:
                        if (this.recovery)
                            return    withRecovery(new ElkhoundParser<>(inputStackFactory, RecoveryParseState.factory(activeStacksFactory, forActorStacksFactory), parseTable, HybridElkhoundStackManager.factory(), NullParseForestManager.factory(), null, ElkhoundReduceManager.factoryElkhound(recoveryReducerFactory()), RecoveryParseFailureHandler.factory(), RecoveryParseReporter.factory()));
                        else
                            return withoutRecovery(new ElkhoundParser<>(inputStackFactory, ParseState.factory(activeStacksFactory, forActorStacksFactory),         parseTable, HybridElkhoundStackManager.factory(), NullParseForestManager.factory(), null, ElkhoundReduceManager.factoryElkhound(reducerFactory()),         DefaultParseFailureHandler.factory(),  EmptyParseReporter.factory()));
                    default: throw new IllegalStateException("Elkhound reducing requires Elkhound stack");
                }
                case Basic: switch(this.stackRepresentation) {
                    case Basic:
                        if (this.recovery)
                            return    withRecovery(new Parser<>(inputStackFactory, RecoveryParseState.factory(activeStacksFactory, forActorStacksFactory), parseTable, BasicStackManager.factory(),  NullParseForestManager.factory(), null, ReduceManager.factory(recoveryReducerFactory()), RecoveryParseFailureHandler.factory(), RecoveryParseReporter.factory()));
                        else
                            return withoutRecovery(new Parser<>(inputStackFactory, ParseState.factory(activeStacksFactory, forActorStacksFactory),         parseTable, BasicStackManager.factory(),  NullParseForestManager.factory(), null, ReduceManager.factory(reducerFactory()),         DefaultParseFailureHandler.factory(),  EmptyParseReporter.factory()));
                    case Hybrid:
                        if (this.recovery)
                            return    withRecovery(new Parser<>(inputStackFactory, RecoveryParseState.factory(activeStacksFactory, forActorStacksFactory), parseTable, HybridStackManager.factory(), NullParseForestManager.factory(), null, ReduceManager.factory(recoveryReducerFactory()), RecoveryParseFailureHandler.factory(), RecoveryParseReporter.factory()));
                        else
                            return withoutRecovery(new Parser<>(inputStackFactory, ParseState.factory(activeStacksFactory, forActorStacksFactory),         parseTable, HybridStackManager.factory(), NullParseForestManager.factory(), null, ReduceManager.factory(reducerFactory()),         DefaultParseFailureHandler.factory(),  EmptyParseReporter.factory()));
                    default: throw new IllegalStateException("Basic reducing requires Basic or Hybrid stack");
                }
            ...
            case Hybrid: switch(this.reducing) {
                case Elkhound: switch(this.stackRepresentation) {
                    case BasicElkhound:
                        if (this.recovery)
                            return    withRecovery(new ElkhoundParser<>(inputStackFactory, RecoveryParseState.factory(activeStacksFactory, forActorStacksFactory), parseTable, BasicElkhoundStackManager.factory(),  HybridParseForestManager.factory(), null, ElkhoundReduceManager.factoryElkhound(recoveryReducerFactory()), RecoveryParseFailureHandler.factory(), RecoveryParseReporter.factory()));
                        else
                            return withoutRecovery(new ElkhoundParser<>(inputStackFactory, ParseState.factory(activeStacksFactory, forActorStacksFactory),         parseTable, BasicElkhoundStackManager.factory(),  HybridParseForestManager.factory(), null, ElkhoundReduceManager.factoryElkhound(reducerFactory()),         DefaultParseFailureHandler.factory(),  EmptyParseReporter.factory()));
                    case HybridElkhound:
                        if (this.recovery)
                            return    withRecovery(new ElkhoundParser<>(inputStackFactory, RecoveryParseState.factory(activeStacksFactory, forActorStacksFactory), parseTable, HybridElkhoundStackManager.factory(), HybridParseForestManager.factory(), null, ElkhoundReduceManager.factoryElkhound(recoveryReducerFactory()), RecoveryParseFailureHandler.factory(), RecoveryParseReporter.factory()));
                        else
                            return withoutRecovery(new ElkhoundParser<>(inputStackFactory, ParseState.factory(activeStacksFactory, forActorStacksFactory),         parseTable, HybridElkhoundStackManager.factory(), HybridParseForestManager.factory(), null, ElkhoundReduceManager.factoryElkhound(reducerFactory()),         DefaultParseFailureHandler.factory(),  EmptyParseReporter.factory()));
                    default: throw new IllegalStateException("Elkhound reducing requires Elkhound stack");
                }
                case Basic: switch(this.stackRepresentation) {
                    case Basic:
                        if (this.recovery)
                            return    withRecovery(new Parser<>(inputStackFactory, RecoveryParseState.factory(activeStacksFactory, forActorStacksFactory), parseTable, BasicStackManager.factory(),  HybridParseForestManager.factory(), null, ReduceManager.factory(recoveryReducerFactory()), RecoveryParseFailureHandler.factory(), RecoveryParseReporter.factory()));
                        else
                            return withoutRecovery(new Parser<>(inputStackFactory, ParseState.factory(activeStacksFactory, forActorStacksFactory),         parseTable, BasicStackManager.factory(),  HybridParseForestManager.factory(), null, ReduceManager.factory(reducerFactory()),         DefaultParseFailureHandler.factory(),  EmptyParseReporter.factory()));
                    case Hybrid:
                        if (this.recovery)
                            return    withRecovery(new Parser<>(inputStackFactory, RecoveryParseState.factory(activeStacksFactory, forActorStacksFactory), parseTable, HybridStackManager.factory(), HybridParseForestManager.factory(), null, ReduceManager.factory(recoveryReducerFactory()), RecoveryParseFailureHandler.factory(), RecoveryParseReporter.factory()));
                        else
                            return withoutRecovery(new Parser<>(inputStackFactory, ParseState.factory(activeStacksFactory, forActorStacksFactory),         parseTable, HybridStackManager.factory(), HybridParseForestManager.factory(), null, ReduceManager.factory(reducerFactory()),         DefaultParseFailureHandler.factory(),  EmptyParseReporter.factory()));
                    default: throw new IllegalStateException("Basic reducing requires Basic or Hybrid stack");
                }
            ...
            case DataDependent:
                ...
                switch(this.stackRepresentation) {
                    case Basic:
                        if (this.recovery)
                            return    withRecovery(new Parser<>(inputStackFactory, RecoveryParseState.factory(activeStacksFactory, forActorStacksFactory), parseTable, BasicStackManager.factory(),  DataDependentParseForestManager.factory(), null, DataDependentReduceManager.factoryDataDependent(recoveryReducerFactory()), RecoveryParseFailureHandler.factory(), RecoveryParseReporter.factory()));
                        else
                    ...
                    case Hybrid:
                        if (this.recovery)
                            return    withRecovery(new Parser<>(inputStackFactory, RecoveryParseState.factory(activeStacksFactory, forActorStacksFactory), parseTable, HybridStackManager.factory(), DataDependentParseForestManager.factory(), null, DataDependentReduceManager.factoryDataDependent(recoveryReducerFactory()), RecoveryParseFailureHandler.factory(), RecoveryParseReporter.factory()));
                        else
            ...
            case LayoutSensitive:
                ...
                switch(this.stackRepresentation) {
                    case Basic:
                        //if (this.recovery)
                        //    return    withRecovery(new Parser<>(inputStackFactoryLS, RecoveryParseState.factory(activeStacksFactory, forActorStacksFactory), parseTable, BasicStackManager.factory(),  LayoutSensitiveParseForestManager.factory(), null, LayoutSensitiveReduceManager.factoryLayoutSensitive(recoveryReducerFactory()), RecoveryParseFailureHandler.factory(), RecoveryParseReporter.factory()));
                        //else
                    ...
                    case Hybrid:
                        //if (this.recovery)
                        //    return    withRecovery(new Parser<>(inputStackFactoryLS, RecoveryParseState.factory(activeStacksFactory, forActorStacksFactory), parseTable, HybridStackManager.factory(), LayoutSensitiveParseForestManager.factory(), null, LayoutSensitiveReduceManager.factoryLayoutSensitive(recoveryReducerFactory()), RecoveryParseFailureHandler.factory(), RecoveryParseReporter.factory()));
                        //else
            ...
            case Composite:
                ...
                switch(this.stackRepresentation) {
                    case Basic:
                        //if (this.recovery)
                        //    return    withRecovery(new Parser<>(inputStackFactoryLS, RecoveryParseState.factory(activeStacksFactory, forActorStacksFactory), parseTable, BasicStackManager.factory(),  LayoutSensitiveParseForestManager.factory(), null, LayoutSensitiveReduceManager.factoryLayoutSensitive(recoveryReducerFactory)_), RecoveryParseFailureHandler.factory(), RecoveryParseReporter.factory()));
                        //else
            ...
            case Incremental:
                ...
                switch(this.stackRepresentation) {
                    case Basic:
                        if (this.recovery)
                            return    withRecovery(new IncrementalParser<>(incrementalRecoveryInputStackFactory, RecoveryIncrementalParseState.factory(activeStacksFactory, forActorStacksFactory), parseTable, BasicStackManager.factory(),  IncrementalParseForestManager.factory(), null, IncrementalReduceManager.factoryIncremental(recoveryReducerFactory()), RecoveryParseFailureHandler.factory(), RecoveryParseReporter.factory()));
                        else
                            return withoutRecovery(new IncrementalParser<>(        incrementalInputStackFactory,         IncrementalParseState.factory(activeStacksFactory, forActorStacksFactory), parseTable, BasicStackManager.factory(),  IncrementalParseForestManager.factory(), null, IncrementalReduceManager.factoryIncremental(reducerFactory()),         DefaultParseFailureHandler.factory(),  EmptyParseReporter.factory()));
                    case Hybrid:
                        if (this.recovery)
                            return    withRecovery(new IncrementalParser<>(incrementalRecoveryInputStackFactory, RecoveryIncrementalParseState.factory(activeStacksFactory, forActorStacksFactory), parseTable, HybridStackManager.factory(), IncrementalParseForestManager.factory(), null, IncrementalReduceManager.factoryIncremental(recoveryReducerFactory()), RecoveryParseFailureHandler.factory(), RecoveryParseReporter.factory()));
                        else
                            return withoutRecovery(new IncrementalParser<>(        incrementalInputStackFactory,         IncrementalParseState.factory(activeStacksFactory, forActorStacksFactory), parseTable, HybridStackManager.factory(), IncrementalParseForestManager.factory(), null, IncrementalReduceManager.factoryIncremental(reducerFactory()),         DefaultParseFailureHandler.factory(),  EmptyParseReporter.factory()));
                    default: throw new IllegalStateException();
                }
        }
        // @formatter:on
