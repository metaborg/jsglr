...
public class ParseNode extends AbstractParseNode {
		...
		for (AbstractParseNode kid : kids){
			if (!kid.isLayout() && !kid.isEmpty() && !kid.isIgnoreLayout()) {
				...
				if (kidLeft != null
						...
						&& (left == null || kidLeft.getColumn() < left
								.getColumn()))
					left = kidLeft;
			}
	...
	/**
	 * Transforms this ambuguity node into a non-ambiguous node by selecting
	...
	public void disambiguate(AbstractParseNode n) {
		...
		assert getChildren().length == 2
				&& (n == getChildren()[0] || n == getChildren()[1]);

		setFields(n.isAmbNode() ? AMB_LABEL : n.getLabel(), n.getChildren(),
				n.getNodeType());
	...
	private void replaceDescendantAt(ParseNode before, ParseNode after,
			int index) {
		AbstractParseNode kid = kids[index];
		if (kid == before) {
			kids[index] = after;
			return; // no further inspection needed since cycles should not
					// occur
		} else if (kid instanceof ParseNode && kid.getLine() == getLine()
				&& kid.getColumn() == getColumn()) {
	...
	/**
	 * Initialize the {@link #isParseProductionChain} method for this node and
	 * any candidate chain nodes below it, without using recursion (which would
			...
			case 2:
				if (current instanceof ParseNode) {
					if (((ParseNode) current).isSetPPC)
						return current.isParseProductionChain() ? null
								: current;
				}
	...
	private void setParseProductionChainUpTo(boolean value,
			AbstractParseNode end) {
		AbstractParseNode current = this;
	...
	public int hashCode() {
		...
		cachedHashCode = result; // Assumption is that hashcode is not set
									// during
									// parsing
		return result;
	}
	 ...
	 * i = 0; i < kids.length; i++) { if(kids[i].isParseNode() ||
