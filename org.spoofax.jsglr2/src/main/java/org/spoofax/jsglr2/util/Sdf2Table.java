...
public class Sdf2Table {
    ...
    synchronized public static void setupSdf2TableInTargetDir() throws URISyntaxException, IOException {
        String targetPath =
            new File(WithGrammar.class.getProtectionDomain().getCodeSource().getLocation().toURI().getPath())
                .getParent(); // Path of the target directory
        ...
        if(!new File(pathInTargetDir).exists()) { // Only copy sdf2table to the target directory once
            ...
            Files.copy(Sdf2Table.getNativeInputStream(), Paths.get(pathInTargetDir),
