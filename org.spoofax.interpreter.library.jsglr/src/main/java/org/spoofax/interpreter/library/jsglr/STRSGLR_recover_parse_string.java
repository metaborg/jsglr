...
public class STRSGLR_recover_parse_string extends JSGLRPrimitive {
    ...
    @Override public boolean call(IContext env, Strategy[] svars, IStrategoTerm[] tvars) throws InterpreterException {
        ...
        } else if(tvars[4].getSubtermCount() == 0 && tvars[4].getTermType() == IStrategoTerm.APPL
            && ((IStrategoAppl) tvars[2]).getConstructor().getName().equals("None")) {
            cursorLocation = -1;
        } else {
            return false;
        }
        ...
        } else if(tvars[2].getSubtermCount() == 0 && tvars[2].getTermType() == IStrategoTerm.APPL
            && ((IStrategoAppl) tvars[2]).getConstructor().getName().equals("None")) {
            startSymbol = null;
        } else {
            return false;
    ...
    private boolean isParseTableTerm(IStrategoTerm pt) {
        return pt.getTermType() == IStrategoTerm.APPL
            && ((IStrategoAppl) pt).getConstructor().getName().equals("parse-table");
    }
    ...
    protected IStrategoTerm call(IContext env, IStrategoString input, ParseTable table, String startSymbol)
