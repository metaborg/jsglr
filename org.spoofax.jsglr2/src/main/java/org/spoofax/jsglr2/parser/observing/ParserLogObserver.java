...
public class ParserLogObserver<ParseForest extends AbstractParseForest, StackNode extends AbstractStackNode<ParseForest>>
    ...
    public void createStackLink(StackLink<ParseForest, StackNode> link) {
        ...
        log("Create link " + id(link) + " from stack " + id(link.from) + " to stack "
            + id(link.to) + " with parse node "
            + (link.parseForest != null ? id(link.parseForest) : "null"));
    }
    ...
    public void actor(StackNode stack, AbstractParse<ParseForest, StackNode> parse, Iterable<IAction> applicableActions) {
        log("Actor for stack " + id(stack) + " (applicable actions: "
